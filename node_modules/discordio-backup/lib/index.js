...
const getBackupData = async (backupID) => {
    return new Promise(async (resolve, reject) => {
        ...
        if (file) {
            // If the file exists
            const backupData = require(`${backups}${node_path_1.sep}${file}`);
...
const fetch = (backupID) => {
    return new Promise(async (resolve, reject) => {
        getBackupData(backupID)
            .then((backupData) => {
            const size = (0, fs_1.statSync)(`${backups}${node_path_1.sep}${backupID}.json`).size; // Gets the size of the file using fs
            const backupInfos = {
                data: backupData,
                id: backupID,
    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            if (!options || options.jsonSave === undefined || options.jsonSave) {
                ...
                // Save the backup
                await (0, promises_1.writeFile)(`${backups}${node_path_1.sep}${backupData.id}.json`, backupJSON, 'utf-8');
            }
            // Returns ID
            resolve(backupData);
        }
...
const remove = async (backupID) => {
    return new Promise((resolve, reject) => {
        try {
            require(`${backups}${node_path_1.sep}${backupID}.json`);
            (0, fs_1.unlinkSync)(`${backups}${node_path_1.sep}${backupID}.json`);
            resolve();
        }
        catch (error) {
...
const setStorageFolder = (path) => {
    if (path.endsWith(node_path_1.sep)) {
